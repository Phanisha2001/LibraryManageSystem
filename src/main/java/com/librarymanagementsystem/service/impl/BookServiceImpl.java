...
public class BookServiceImpl implements BookService {
	...
	@Override
	public void borrowBookByName(String name) {
		var book = bookRepository.findByName(name)
				.orElseThrow(() -> new NotFoundException(String.format("Book not found with name %d", name)));
		if (book.getQuantity() <= 0)
			throw new NotFoundException(String.format("Book with name - %d is not available anymore.", name));
